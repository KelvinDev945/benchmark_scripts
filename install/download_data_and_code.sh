#!/bin/bash
# 下载基准测试需要的模型/数据集/评测代码——无卡（CPU）阶段就能跑，提前做好省挂卡计费时间
# 用法：DATA_DIR=/root/rivermind-data bash download_data_and_code.sh
set -e

echo '=== 0. 加载统一分发源配置 ==='
source "$(dirname "${BASH_SOURCE[0]}")/sources.sh"

DATA_DIR="${DATA_DIR:-/root/rivermind-data}"
mkdir -p "$DATA_DIR/models" "$DATA_DIR/datasets"

echo '=== 1. 确保 uv 可用 ==='
if ! command -v uv >/dev/null 2>&1; then
    echo "尝试官方独立安装脚本（走 GitHub Releases CDN，部分国内网络环境会很慢/卡住，最多等${UV_INSTALL_TIMEOUT}秒）"
    if timeout "$UV_INSTALL_TIMEOUT" bash -c 'curl -LsSf https://astral.sh/uv/install.sh | sh' 2>&1 | tail -5; then
        export PATH="$HOME/.local/bin:$PATH"
        grep -q '.local/bin' ~/.bashrc 2>/dev/null || echo 'export PATH="$HOME/.local/bin:$PATH"' >> ~/.bashrc
    fi
fi
if ! command -v uv >/dev/null 2>&1; then
    echo "官方安装脚本超时/失败，改用 apt装pip + pip($PYPI_MIRROR)装uv 这条更快的路径"
    apt-get update -qq && apt-get install -y -qq python3-pip
    python3 -m pip install -qqq uv -i "$PYPI_MIRROR"
fi
echo "uv: $(uv --version)"

echo '=== 1b. 检查/装好 modelscope + huggingface_hub（下载模型/数据集要用） ==='
if ! python3 -c "import modelscope" >/dev/null 2>&1; then
    uv pip install --system -qqq -i "$PYPI_MIRROR" modelscope huggingface_hub
fi

echo '=== 2. 下载 base 模型 DeepSeek-R1-Distill-Qwen-1.5B ==='
echo "    PREFER_MODELSCOPE_FOR_MODELS=$PREFER_MODELSCOPE_FOR_MODELS —— 模型优先走ModelScope（国内更快），"
echo "    找不到再退回 huggingface_hub（经 HF_ENDPOINT=$HF_ENDPOINT 镜像下载）"
if [ ! -d "$DATA_DIR/models/DeepSeek-R1-Distill-Qwen-1.5B" ]; then
    if [ "$PREFER_MODELSCOPE_FOR_MODELS" = true ] && python3 -c "
from modelscope import snapshot_download
snapshot_download('deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B', local_dir='$DATA_DIR/models/DeepSeek-R1-Distill-Qwen-1.5B')
"; then
        echo "已通过 ModelScope 下载完成"
    else
        echo "ModelScope 下载失败/未启用，改走 huggingface_hub（经 $HF_ENDPOINT 镜像）"
        python3 -c "
from huggingface_hub import snapshot_download
snapshot_download(repo_id='deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B', local_dir='$DATA_DIR/models/DeepSeek-R1-Distill-Qwen-1.5B')
"
    fi
else
    echo "已存在，跳过：$DATA_DIR/models/DeepSeek-R1-Distill-Qwen-1.5B"
fi

echo '=== 3. 下载训练数据集 open-r1/DAPO-Math-17k-Processed ==='
echo "    ModelScope 上没有这个数据集镜像，走 huggingface_hub（经 HF_ENDPOINT=$HF_ENDPOINT 镜像）"
if [ ! -d "$DATA_DIR/datasets/DAPO-Math-17k-Processed" ]; then
    python3 -c "
from huggingface_hub import snapshot_download
snapshot_download(
    repo_id='open-r1/DAPO-Math-17k-Processed',
    repo_type='dataset',
    local_dir='$DATA_DIR/datasets/DAPO-Math-17k-Processed',
)
"
else
    echo "已存在，跳过：$DATA_DIR/datasets/DAPO-Math-17k-Processed"
fi

echo '=== 4. clone JustRL 仓库（评测脚本 + 9个benchmark数据） ==='
echo "    先测本实例GitHub直连是否可用，不可用则走 GITHUB_PROXY_PREFIX=$GITHUB_PROXY_PREFIX（每台实例网络环境不一样，详见环境文档）"
if curl -s -m 8 -o /dev/null -w '%{http_code}' https://github.com | grep -q '200'; then
    GITHUB_PREFIX=""
    echo "GitHub 直连可用"
else
    GITHUB_PREFIX="$GITHUB_PROXY_PREFIX"
    echo "GitHub 直连不可用，改用代理：$GITHUB_PREFIX"
fi

if [ ! -d "$DATA_DIR/JustRL" ]; then
    git clone --depth 1 "${GITHUB_PREFIX}https://github.com/thunlp/JustRL.git" "$DATA_DIR/JustRL"
else
    echo "已存在，跳过：$DATA_DIR/JustRL"
fi

echo '=== 全部完成 ==='
echo "模型: $DATA_DIR/models/DeepSeek-R1-Distill-Qwen-1.5B"
echo "数据: $DATA_DIR/datasets/DAPO-Math-17k-Processed"
echo "代码: $DATA_DIR/JustRL"
