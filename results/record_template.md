...
| 云端 RTX 4090 48G（sc02，2026-08-01） | 132.3-139 TFLOPS（稳态约132-133 TFLOPS，对比"真实RTX 4090"165TFLOPS约80%；errors:0，峰值温度76°C；实际可用显存48101MB，压测占用43291MB） | 470.91 GB/s（47%，对比真实4090 1008GB/s） | H→D 25.09 GB/s，D→H 26.28 GB/s，双向22.61GB/s | 算力/带宽/PCIe三项都跟已有"云端 4090"（139.7TFLOPS/459GB/s/25.90GB/s）几乎完全一致，进一步验证"云端虚拟化平台统一限速规律"——同芯片只是显存从24G加到48G，核心算力/带宽不受影响 |
| （新测的卡） | | | | |
...
| A100-PCIE-40GB（fj02，2026-07-21，纯vLLM推理吞吐，len16384） | — | — | **1026.70 tokens/s** | N/A | N/A | 34.71GB(84.7%)，峰值利用率91%，平均利用率**49.3%**（比len8192更高）；对比4090历史975.21 tokens/s，快5.3% |
| **RTX 4060 Ti（kelvin-linux，2026-07-22，真机非云端）纯vLLM推理吞吐，NUM_PROMPTS=8，len8192** | — | — | **454.53 tokens/s** | N/A | N/A | 真实峰值15.52GB(**94.7%**)，峰值利用率**99%**，平均利用率**87.7%**（远高于4090/A100的35.8%）；吞吐是4090的41.3%/A100的40.5%，比硬件跑分差距(约24-27%)接近得多——16GB小显存被8并发逼到接近满载，大显存卡在低并发下反而没吃饱 |
| A100-PCIE-40GB（fj02，2026-07-21，纯训练`train_only_benchmark.py`，seq4096/bs48，LOAD_VLLM_ENGINE=0） | — | 16.08 | — | N/A | N/A | 真实峰值23.251GB(**仅58.1%**，4090同配置到93.9%)，峰值利用率100%，平均利用率69.7%；对比4090的20.28s，**A100快20.7%**——本轮唯一测出干净算力优势的场景 |
| A100-PCIE-40GB（fj02，2026-07-21，纯训练，seq8192/bs24，LOAD_VLLM_ENGINE=0） | — | 17.82 | — | N/A | N/A | 真实峰值23.25GB(58.1%，跟seq4096组几乎一致)，峰值利用率100%，**平均利用率29.3%**(明显低于seq4096组的69.7%，原因未查明，"梯度offload导致"这个归因已被源码验证推翻，见obsidian笔记)；对比4090的22.32s，**A100快20.2%**（跟seq4096组的20.7%高度一致） |
...
## 五、智川云价格快照 + 带宽性价比分析（2026-08-01）

> ⚠️ 价格随时间波动（"限时优惠"/库存变化），这里只是某一时刻的快照，
> 用之前先去智川云控制台确认当前实际价格，不要直接当作长期有效数字引用。

| GPU | 显存 | 地区 | 按量价格 | 库存 |
|---|---|---|---|---|
| A100-SXM4-40GB | 40GB | 江苏 | ¥2.39/时 | 立即创建 |
| A100-PCIE-40GB | 40GB | 福建2区（限时优惠） | ¥1.68/时 | 立即创建 |
| A100-SXM4-80GB | 80GB | 上海1区 | ¥5.6/时 | 到货提醒 |
| RTX 4090 | 24GB | 新疆/海南/福建 | ¥1.28~1.46/时 | 立即创建 |
| RTX 4090-48G | 48GB | 新疆 | ¥1.89/时 | 立即创建 |
| RTX 4090-48G | 48GB | 四川2区 | ¥1.98/时 | 到货提醒 |
| RTX 4090 Lite | 24GB | 新疆 | ¥1.28/时 | 立即创建 |
| RTX 5090 | 32GB | 辽宁/安徽 | ¥1.89/时 | 到货提醒 |
| RTX 5090 | 32GB | 四川2区 | ¥1.98/时 | 立即创建 |
| RTX 3090 | 24GB | 四川/浙江 | ¥0.99/时 | 立即创建 |
| RTX 3090 Lite | 24GB | 浙江 | ¥0.88/时 | 到货提醒 |
| Tesla T4 | 16GB | 上海2区 | ¥0.52/时 | 立即创建 |

**带宽性价比（取"当前实际能立即创建"的最低价档位，结合本表第一节实测带宽）**：

| GPU | 带宽 | 按量价格 | 带宽/元 |
|---|---|---|---|
| A100-PCIE-40GB（福建2区） | 686.76 GB/s | ¥1.68/时 | **408.8** |
| RTX 5090（四川2区，¥1.89那档缺货） | 761 GB/s | ¥1.98/时 | 384.3 |
| RTX 4090-48G（新疆） | 470.91 GB/s | ¥1.89/时 | 249.2 |

**结论**：截至这次快照，A100-PCIE-40GB 综合看是 vLLM 推理性价比最高的选择——
带宽/元最高、显存(40GB)比5090(32GB)大（KV cache 容量更有优势）、按量价格也最低。
RTX 4090-48G 虽然显存最大(48GB)，但带宽只有470.91GB/s，拖累了整体的"元/带宽"比值，
三者里性价比最差；不过如果工作负载对显存容量的需求超过40GB上限（比如更大模型/更长
上下文/更高并发），48GB的空间优势可能仍然值得多花的这点钱，需要按具体场景权衡。
