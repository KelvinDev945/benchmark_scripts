...
def query_gpu_memory_used_gb():
    ...
    return max(int(line.strip()) for line in out.stdout.strip().splitlines()) / 1024  # MiB -> GiB
...
def snapshot_memory(tag):
    ...
    nvidia_smi_used = query_gpu_memory_used_gb()
    ...
    print(f"[memory] {tag}: allocated={allocated:.2f}GB reserved={reserved:.2f}GB "
          ...
          + ("  ⚠️standby模式下max_allocated可能失真，以nvidia_smi_used为准" if USE_VLLM_STANDBY and USE_VLLM else ""))
